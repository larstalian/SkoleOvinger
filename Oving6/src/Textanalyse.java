...
public class Textanalyse {
  ...
  public Textanalyse(String tekst) {
    ...
    System.out.println(
        "Antall forekomster av spesialtegn: " + antallTegn[29]); // Lager unntak for spesialtegn
  }

  public void sumBokstaver() {
    System.out.println(
        "Totalt antall bokstaver er: " + (Arrays.stream(antallTegn).sum() - antallTegn[29]));
  }
  ...
  public void spesialtegnProsent() {
    double[] arrayDouble =
        Arrays.stream(antallTegn).asDoubleStream().toArray(); // kopierer/formatterer til double
    System.out.printf(
        "Spesialtegn utgjør %.2f%% av teksten",
        100 * (arrayDouble[29] / (Arrays.stream(arrayDouble).sum())));
  }
    ...
    for (int i = 0; i < tekst.length(); i++) {
      int bokstavIndex =
          alfabet.indexOf(
              tekst
                  .toLowerCase()
                  .charAt(i)); // int som returnerer indexen til karakteren i teksten på posisjon i
  ...
  public void antallSpesifikkBokstav() {
    ...
    for (int i = 0; i < tekst.length(); i++) {
      int bokstavIndex =
          alfabet.indexOf(tekst.toLowerCase().charAt(i)); // indexen til bokstaven på posisjon i
  ...
  public void forekommerOftest() {
    // denne metoden bruker en løkke til å gå igjennom antallTegn og finner den høyeste verdien.
    // Deretter finner bokstaven på den plassen.
    int hoyest = -1;
    ...
    for (int i = 0; i < (antallTegn.length - 1); i++) {
      ...
      } else if (antallTegn[i]
          == hoyest) { // finnes det flere bokstaver med samme verdi legges de til i Stringbuilderen
        hoyestBokstav.append(", ").append(alfabet.charAt(i));
      }
    }
    System.out.println(
        "Bokstaven(e) som forekommer oftest er \"" + hoyestBokstav + "\" (" + hoyest + " ganger)");
  }
}
