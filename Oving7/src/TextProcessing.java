...
public record TextProcessing(String text) {
  ...
  public int getNumberOfWords() {
    // Splits the text at whitespaces into arrays. Then counts the array instances and returns the
    // int.
    return (int) Arrays.stream(text().split(" ")).count();
  }

  public double getAverageWordLength() {
    // Removes "," ".". Then splits into an array on the whitespaces and returns the average array
    // instance length
    String[] words =
  ...
  public double getAveragePeriod() {
    // Splits the text on whitespaces in variable "words" and ?.:! in the variable "period". Then
    // divides the word length with the period length
